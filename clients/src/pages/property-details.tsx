...
import {Delete, ChatBubble, Edit, Phone, Place, Star} from "@mui/icons-material";
...
const propertyDetails = () => {
  ...
  const { data: user } = useGetIdentity();    //id of the current resource we are currently trying to view
  const { queryResult } = useShow();          
  const { mutate } = useDelete();             //this will help to quickly delete a resource
  const { id } = useParams();
    ...
    >
        <Box flex={1} maxWidth={764}>
            ...
            <Box mt="15px">
              ...
              {/* <Rating value={value} onChange={(e, newValue) => setValue(newValue)} precision={0.5}/> */}
